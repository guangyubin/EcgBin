...
function [maxloc,qrsheight, qrswidth,sign] = qrs_detect5_1(ecg,varargin)
...
try
    ...
    [b,a] = butter(3,[4 20]/(fs/2),'bandpass');
    ...
    if (sum(abs(ecg-median(ecg))>MIN_AMP)/NB_SAMP)>0.02
        ...
        if debug==1
            ...
            subplot(211); plot(ecg); hold on;plot(rloc,ecg(rloc),'.');hold off;
