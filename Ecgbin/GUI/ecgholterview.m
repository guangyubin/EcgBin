...
list = dir('D:\MGCDB\CAREB1');
for ii = 1:length(list)
    if list(ii).isdir==1 && isempty(strfind(list(ii).name,'.'))
        m = m +1;
        drt{m} = fullfile('D:\MGCDB\CAREB1',list(ii).name);
    end
end
list = dir('D:\MGCDB\CAREB0\DataFromServer');
for ii = 1:length(list)
    if list(ii).isdir==1 && isempty(strfind(list(ii).name,'.'))
        m = m +1;
        drt{m} = fullfile('D:\MGCDB\CAREB0\DataFromServer',list(ii).name);
    end
end
...
qpos =  repmat(double(handles.ECG.AnnBeat.time),[1 5])  +  double(handles.ECG.AnnBeat.qrs(:,[4 5 6 7 14]))+1 ;
...
if ~isempty(handles.ECG.AnnBeat3)
    hold on ;
    plot_ecg_chan1(handles.ECG.ecg1,handles.ECG.heasig.freq,handles.ECG.AnnBeat3.time,handles.ECG.AnnBeat3.anntyp,pos,15,0.85,'ok');
end
hold off;
...
hold on;handles.line3 = plot(t, handles.ECG.Report.AFIndex(1:length(t)),'b');
handles.line4 = plot(t, handles.ECG.Report.PVCIndex,'.r');
if length(handles.ECG.Report.STValue) > 1
    handles.line5 = plot(t, handles.ECG.Report.STValue(1:length(t)),'k');
end

 plot_vbar(handles.ECG.ref_AF_Label,250, -40, max(t));
 
plot_vbar(handles.ECG.Report.Event_AF ,1, -60, max(t),'k');

% legend('hr','AFIndex','PVCIndex','STValue');
hold off;
%     end
set(handles.line2,'XData',[pos pos]);
set(handles.line2,'YData',[-100 200]);
...
[handles.ECG.AnnBeat2 , handles.ECG.ref_AF_Label ]= loadmitqrs([fname(1:length(fname)-4) '.atr']);
handles.ECG.AnnBeat3 = loadmitqrs([fname(1:length(fname)-4) '.rqrs']);
...
if isfield(handles.ECG.Report,'PatientInfo')
set(handles.textInfo,'string',handles.ECG.Report.PatientInfo);
end
...
handles.filepath = contents{get(hObject,'Value')};
 if ~isempty(listname(handles.filepath))
    set(handles.FileListBox,'string', listname(handles.filepath));
