function  [onset, offset] = find_slope_position(x, findmark,fs)
% dx = abs(dx);
% maxSlope = abs(maxSlope);
dx = diff(x);
[maxSlope ,maxSlopeI] = max_pos(dx,findmark -0.15*fs,findmark +0.15*fs);
[minSlope ,minSlopeI]= min_pos(dx,findmark -0.15*fs,findmark +0.15*fs);
