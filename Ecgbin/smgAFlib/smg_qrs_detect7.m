...
function [maxloc,qrsheight, qrswidth,sign,noise_qrs] = smg_qrs_detect7(ecg,varargin)
...
%                 end
        ...
        index_qrswidth = find( qrs_width > floor(1.6*mean_qrs_width));
        [~,index_length]=size(index_qrswidth);
        if index_length >0
            for ii=1:index_length
                poss_reg((left(index_qrswidth(ii)):right(index_qrswidth(ii)))) = 0 ;
                noise_qrs=[noise_qrs ((left(index_qrswidth(ii))+right(index_qrswidth(ii))))/2];
            end
        end
...
%             tmpdiffecg = diff(ecg(rloc(ii)-3:rloc(ii)+3));
       ...
       %%
        %去掉异常的QRS
        noise_qrs=[noise_qrs rloc(index123)];
        noise_qrs = unique(noise_qrs);
        maxloc(index123)=[];
        ...
        index123 = find(maxloc > 0.15*fs & maxloc < length(ecg)-0.15*fs);
        maxloc = maxloc(index123);
        qrsheight = qrsheight(index123);
        qrswidth = qrswidth(index123);
