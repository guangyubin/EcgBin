...
function [wavepos, QRSampl,Tampl] = ptdetector_bin3(x,fs,findmark,rr,fig)
...
Qflat1 = findFlatLeft_200(x,fs,floor(0.020*fs),0.02*QRSampl,ma, floor(0.12*fs),'left');
...
str = sprintf('ma = %d Qflat1 = %d  Qsl = %d  Q_a = %d \n ' ,ma,  Qflat1,Qsl,Q_a);
%disp(str);

Sis = findFlatLeft_200(x,fs,floor(0.02*fs),0.02*QRSampl,ma, floor(0.15*fs+0.5),'right');
Qe_flat = Sis;
...
Q_e = Qe_onst;
str = sprintf('ma = %d  Sis = %d  Qe_flat = %d  Qe_slop = %d   Q_e = %d\n  ' ,ma,Sis,  Qe_flat,Qe_slop,Q_e);
%disp(str);
%-----------Find P -----------------------------------------------------------------
...
Qflat0 = findFlatLeft_200(x,fs,floor(0.020*fs),0.04*QRSampl,Qflat1-floor(0.1*fs), floor(0.12*fs),'left');
str = sprintf('Qflat0 = %d   Qflat1 = %d\n  ' ,Qflat0 , Qflat1);
%disp(str);
...
% tic
% Pis = findFlatLeft_200(Wing,fs,floor(0.020*fs),floor(0.04*Wing(Pp)),Pp, floor(0.05*fs),'right');
Pis = Pp+10;
...
str = sprintf('Pp = %d Pis = %d Pth = %d  P_Off = %d\n  ' ,Pp ,Pis, Pth ,P_Off);
%disp(str);


% Pis = findFlatLeft_200(Wing,fs,floor(0.020*fs),0.04*Wing(Pp),Pp, floor(0.06*fs),'left');
Pis = Pp - 10;
[Mi2 mi2]=max(Wing(Pis:Pp));
Pth=Pis+mi2-1;
P_On = findAnglesDiffMost(x,fs,floor(0.01*fs),Pis,Pth);
str = sprintf('Pp = %d Pis = %d Pth = %d  P_On = %d\n  ' ,Pp ,Pis, Pth ,P_On);
%disp(str);
if fig==1
...
if length(Sign)>0;
 ...
 str = sprintf('Tp = %d Tth = %d Tis = %d  Tend = %d \n  ' ,Tp ,Tth, Tis,T_end);
%disp(str);
